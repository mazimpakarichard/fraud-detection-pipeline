...
class IsolationForestModel:
    ...
    def fit(
        ...
        logger.info(
            ...
            top_features=dict(zip(
                ...
                self.feature_importances_[:5].tolist()
            )),
        )
    ...
    def get_top_anomaly_features(
        ...
        importances = self.feature_importances_ if self.feature_importances_ is not None else np.ones(X_scaled.shape[1])
        ...
        for i in range(len(X_array)):
            ...
            top_features = [
                (self.feature_names[idx], float(sample_contrib[idx]))
