...
class FeatureEngineer:
    ...
    def _compute_risk_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if "billing_country" in df.columns and "shipping_country" in df.columns:
            features["country_mismatch"] = (df["billing_country"] != df["shipping_country"]).astype(
                int
            )
        ...
        if "merchant_category" in df.columns:
            features["high_risk_category"] = (
                df["merchant_category"].isin(self.HIGH_RISK_CATEGORIES).astype(int)
            )
    ...
    def _rolling_count(self, df: pd.DataFrame, entity_col: str, window_hours: int) -> pd.Series:
        """Count transactions per entity in rolling window."""
        result = pd.Series(0, index=df.index)

        # Group by entity
        for _entity_val, group in df.groupby(entity_col):
            ...
            for idx, row in sorted_group.iterrows():
                ...
                count = (
                    (sorted_group["timestamp"] >= window_start)
                    & (sorted_group["timestamp"] < row["timestamp"])
                ).sum()
    ...
    def _rolling_amount_stats(
        self, df: pd.DataFrame, entity_col: str, window_hours: int
    ) -> dict[str, pd.Series]:
        ...
        for _entity_val, group in df.groupby(entity_col):
            ...
            for idx, row in sorted_group.iterrows():
                window_start = row["timestamp"] - pd.Timedelta(hours=window_hours)
                mask = (sorted_group["timestamp"] >= window_start) & (
                    sorted_group["timestamp"] < row["timestamp"]
                )
                window_amounts = sorted_group.loc[mask, "amount"]
    ...
    def _rolling_unique_count(
        self, df: pd.DataFrame, entity_col: str, target_col: str, window_hours: int
    ) -> pd.Series:
        ...
        for _entity_val, group in df.groupby(entity_col):
            ...
            for idx, row in sorted_group.iterrows():
                ...
                unique_count = sorted_group.loc[mask, target_col].nunique()
